...
class AlgebraicGraph a where 
    ...
    nodes :: AlgebraicGraph a V E -> [vertex]
-}
...
outNeighbors node graph = case graph of
    ...
    (Connect a b) -> let
        vertices = (nodes a)
        cond = (elem node vertices)
        in (if cond then S.union (nodes b) (outNeighbors node a) else outNeighbors node b)
    _ -> S.empty
...
inNeighbors node graph = case graph of
    ...
    (Connect a b) -> let
        vertices = (nodes b)
        cond = (elem node vertices)
        in (if cond then S.union (nodes a) (inNeighbors node b) else inNeighbors node a)
    _ -> S.empty
...
instance Num a => Num (AlgebraicGraph a) where
    fromInteger number = Node $ fromInteger number
    (+) = Overlay
    (*) = Connect
...
instance Show a => Show (AlgebraicGraph a) where
    show graph = auxshow graph where
        auxshow g = case g of
            Empty -> ""
            Node a -> show a
            (Overlay a b) -> "(" ++ auxshow a ++ "+" ++ auxshow b ++ ")"
            (Connect a b) -> "(" ++ auxshow a ++ "*" ++ auxshow b ++ ")"
...
splitNode node targets = let
    target_graph = foldl (\acc x -> Overlay (Node x) acc) Empty targets
    in (extend (\n -> if n == node then target_graph else Node n))
