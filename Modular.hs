...
-}
mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f xs = mapSingleaux 1
    where
        mapSingleaux n
                    | n == (length xs + 1) = []
                    | otherwise = let
                        first_part = take (n - 1) xs
                        last_part = drop n xs
                        new_elem = f $ xs !! (n - 1)
                        res_list = (first_part ++ [new_elem]) ++ last_part
                        in (res_list : (mapSingleaux $ n + 1))
...
partitions xs = case xs of
    [] -> [[]]
    (y : ys) -> (\x y -> x ++ y) [[y] : zs | zs <- partitions ys] 
                                 (concatMap (\x -> x) [mapSingle ([y]++) zs | zs <- partitions ys])
...
isModule set graph = let
    set_list = S.toList set
    nei_list = map (\x -> (,) (S.toList $ outNeighbors x graph) (S.toList $ inNeighbors x graph)) set_list
    xs = map (\(out_list, in_list) -> (,) (filter (\x -> not $ elem x set_list) out_list) (filter (\x -> not $ elem x set_list) in_list)) nei_list
    in (all (== head xs) (tail xs))
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = res where
    list_partition = foldl (\acc x -> acc ++ [x]) [] (S.toList partition)
    all_modules = foldl (\acc set -> (isModule set graph) : acc) [] list_partition
    res = foldl (\acc x -> acc && x) True all_modules
...
maximalModularPartition :: Ord a
                        ...
                        -> Graph a
                        -> Partition a
maximalModularPartition partitions graph = res where
    n_list = filter (\y -> isModularPartition y graph) $ filter (\x -> (length x) > 1) (sortBy (\e1 e2 -> compare (length e1) (length e2)) partitions)
    res = head n_list
